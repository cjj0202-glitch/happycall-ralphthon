...
const args = process.argv.slice(2), refIndex = args.indexOf('--source-ref'), dependencyIndex = args.indexOf('--dependencies-ref');
const sourceRef = refIndex < 0 ? null : args[refIndex + 1];
const dependencyRef = dependencyIndex < 0 ? pinnedMain : args[dependencyIndex + 1];
if (refIndex >= 0 && (!sourceRef || sourceRef.startsWith('--'))) throw new Error('--source-ref needs a local Git revision');
if (!dependencyRef || dependencyRef.startsWith('--')) throw new Error('--dependencies-ref needs a local Git revision');
...
const source = readAt(sourcePath), fixtureText = readAt('data/fixtures/cases.json', dependencyRef), overlayText = readAt('data/overlays/pc4-tms.json', dependencyRef);
...
const report = {
  ...
  startedAt: new Date().toISOString(), sourceRef: sourceRef || 'working-tree', dependencyRef, baselineExpectedToFail: args.includes('--baseline'),
...
if (/import\s*\{[^}]*\bApiError\b[^}]*\}\s*from\s*['"]@\/lib\/api['"]/.test(source)) {
  const apiText = readAt('apps/web/lib/api.ts', dependencyRef), apiRef = dependencyRef;
  const apiAst = ts.createSourceFile('api.ts', apiText, ts.ScriptTarget.Latest, true, ts.ScriptKind.TS);
  const declaration = apiAst.statements.find(node => ts.isClassDeclaration(node) && node.name?.text === 'ApiError');
  const localApiAst = ts.createSourceFile('api.ts', readAt('apps/web/lib/api.ts', null), ts.ScriptTarget.Latest, true, ts.ScriptKind.TS);
  const workingTreeClassAvailable = localApiAst.statements.some(node => ts.isClassDeclaration(node) && node.name?.text === 'ApiError');
  report.limitations.push(`${workingTreeClassAvailable ? 'Working-tree ApiError class exists, but import resolution is not tested' : 'Working-tree ApiError import is absent'}: actual dependencyRef class is used explicitly. Working-tree import/typecheck parity is NOT_RUN here; do not count this as a successful app build.`);
  ...
  report.apiDependency = { mode: 'actual-class-AST-extraction', ref: apiRef, fileSha256: sha(apiText), classSha256: sha(apiClassSource), workingTreeClassAvailable, workingTreeParity: false };
