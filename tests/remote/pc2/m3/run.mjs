...
try{
  ...
  try{
    const meta=JSON.parse(await readFile(path.join(OUT,'server.json'),'utf8'));
    execution.serverFinalProof={stopped:meta.stopped===true,counts:meta.counts};
    assert.equal(meta.stopped,true,'Host must stop and flush its final evidence');
    assert.deepEqual(meta.counts,{paidAttempts:0,keyReadAttempts:0,externalAttempts:0});
    assert.ok(execution.cleanup.some(c=>c.browser==='closed'&&c.disconnected===true));
    assert.ok(execution.cleanup.some(c=>c.server==='exited'&&c.code===0&&c.signal===null));
  }catch(e){execution.cleanupFailure=e.stack||String(e);}
...
if(execution.fatal||execution.integrityFailure||execution.cleanupFailure||report?.fatal||report?.summary.failed||report?.summary.passed!==14)process.exitCode=1;
