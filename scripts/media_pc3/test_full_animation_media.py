...
SCRATCH = ROOT / ".local/pc3-m6-intake"
...
def fake_mp4(faststart=True,*,width=1920,height=1080,frames=288,movie_duration=12000,media_duration=147456):
    """Only box-order test data; deliberately not a decodable MP4."""
    def box(kind,payload):
        return struct.pack(">I",8+len(payload))+kind+payload
    ftyp=box(b"ftyp",b"isom\0\0\0\0isomiso2avc1mp41")
    mvhd=box(b"mvhd",bytes(12)+struct.pack(">II",1000,movie_duration)+bytes(80))
    ...
    stsd=box(b"stsd",bytes(4)+struct.pack(">I",1)+box(b"avc1",bytes(sample)))
    stts=box(b"stts",bytes(4)+struct.pack(">III",1,frames,512))
    stbl=box(b"stbl",stsd+stts)
    moov=box(b"moov",mvhd+box(b"trak",box(b"mdia",mdhd+hdlr+box(b"minf",stbl))))
...
class FullAnimationMediaTests(unittest.TestCase):
    ...
    def rejected(self,name,output=None,**kwargs):
        ...
        if output.is_dir() and output!=self.package:
            ...
            self.assertFalse((output/"sorter-demo.tracks.descriptor.json").exists())
    ...
    def test_late_write_failure_removes_owned_success_markers(self):
