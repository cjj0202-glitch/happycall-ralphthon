...
function Desk({ caseData: c, mode, fallback, onUpdate, onSave, onView, onToast }: { caseData: CaseData; mode: Mode; fallback: boolean; onUpdate: (c: CaseData) => void; onSave: (id: string, p: Record<string, unknown>) => Promise<CaseData>; onView: (v: View) => void; onToast: (s: string) => void }) {
  ...
  const persist = async (transfer = false) => {
    ...
    try {
      ...
      const saved = await onSave(c.id, { expectedRevision: formRevision, intake: { ...form, request: nextRequest, quantity: form.quantity === '' || form.quantity === null ? null : Number(form.quantity) }, departmentId: department || null, reviewConfirmed: confirmed, status: transfer ? 'handed_off' : 'review' });
      setFormRevision(saved.revision);
      setForm(old => ({ ...old, request: nextRequest })); setQuestion('');
      onToast(transfer ? '확인한 접수 내용과 근거를 센터에 전달했습니다.' : '상담원이 편집한 접수 정보를 저장했습니다.');
      if (transfer) onView('center');
  ...
  return <div className="desk">
    ...
    <div className="integrated-review">
      ...
      <section id="intake-editor" className="panel refinement-panel"><div className="panel-heading"><div><span className="small-kicker">02 / REFINE & REVIEW</span><h3>AI 정제와 접수 확인</h3></div>{resultMode && <span className={`badge ${resultMode === 'demo-live' ? 'success' : 'info'}`}>{resultMode === 'demo-live' ? '실제 AI 분석' : '저장 결과 재생'}</span>}</div>
        ...
        <div className="form-grid"><label>점포코드 <span className="required">필수</span><input value={form.storeId || ''} onChange={e => field('storeId', e.target.value)} placeholder="점포코드 확인"/></label><label>상품·문의 대상 <span className="required">필수</span><input value={form.subject || ''} onChange={e => field('subject', e.target.value)} placeholder="상품명 또는 문의 대상"/></label><label>수령 수량(경영주 진술)<input type="number" min="0" value={form.quantity ?? ''} onChange={e => field('quantity', e.target.value)} placeholder="미확인"/></label><label>수령 단위<input value={form.unit || ''} onChange={e => field('unit', e.target.value)} placeholder="개 / 박스 · 미확인"/></label><label className="full">요청사항<textarea value={form.request || ''} onChange={e => field('request', e.target.value)} placeholder="경영주가 요청한 내용을 확인해 주세요." rows={3}/></label></div>
...
function Owner({ cases, selected, onSelect, onCreated, onDesk, fallback }: { cases: CaseData[]; selected: string; onSelect: (s: string) => void; onCreated: (c: CaseData) => void; onDesk: () => void; fallback: boolean }) {
  const [storeId, setStoreId] = useState(''); const [subject, setSubject] = useState(''); const [text, setText] = useState(''); const [type, setType] = useState<'missing' | 'wrong'>('missing');
