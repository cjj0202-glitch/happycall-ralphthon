...
import { defaultQueue, departmentName, deskStep, filterCases, inQueue, isEvidenceEditable, nextAction, queueForCase, queues, roleNames, roleView, type QueueId, type WorkRole } from '@/lib/workflow';
...
export default function Home() {
  ...
  const visibleCases = filterCases(cases, role, queue, query);
  ...
  const changeRole = (nextRole: WorkRole) => {
    setRole(nextRole); setView(roleView[nextRole]); setQueue(defaultQueue(nextRole)); setQuery(''); setToast('');
  };
  ...
  const selectCase = (id: string) => {
    setSelected(id); setView(roleView[role]); setToast('');
  ...
  return <div className="app-shell">
    ...
    <main id="main" className="main-content">
      ...
      <div className="workspace-sync">
        ...
        {role === 'counselor' && !logistics && <div className="mode-control"><label htmlFor="analysis-mode">분석 방식</label><select id="analysis-mode" value={mode} onChange={e => setMode(e.target.value as Mode)}><option value="replay">저장 결과 재생 · API 호출 없음</option><option value="demo-live">실제 AI 분석 · API 사용</option></select></div>}
      ...
      {loading && !cases.length ? <div className="empty-state" role="status"><div className="spinner"/><h2>접수 건을 불러오고 있습니다</h2><p>서버의 최신 처리 상태를 확인합니다.</p></div> : role === 'owner' ? <Owner cases={cases} selected={selected} onSelect={setSelected} onCreated={c => { update(c); setSelected(c.id); setToast(`${c.id} 접수가 등록되었습니다.`); }} onToast={setToast} onDesk={() => navigate('desk')} fallback={fallback}/> : <div className={`role-workspace${logistics ? ' logistics-workspace' : ''}`}>
        {!logistics && <aside id="work-queue" tabIndex={-1} className="work-queue" aria-label={`${roleNames[role]} 접수 목록`}>
          <div className="queue-heading"><h2>처리할 접수</h2><span className="small muted">전체 {cases.length}건</span></div>
          <div className="queue-filters" aria-label="처리 상태">{queues(role).map(item => <button key={item.id} aria-pressed={queue === item.id} onClick={() => { setQueue(item.id); setView(roleView[role]); }} className={queue === item.id ? 'selected' : ''}>{item.label}<span>{cases.filter(c => inQueue(c, role, item.id)).length}</span></button>)}</div>
          <label className="queue-search"><span className="sr-only">접수 검색</span><input type="search" placeholder="점포, 제목, 접수번호 검색" value={query} onChange={e => setQuery(e.target.value)}/></label>
          <p className="queue-result" role="status">{queues(role).find(item => item.id === queue)?.label} · {visibleCases.length}건</p>
          <div className="queue-list">{visibleCases.map(c => <button key={c.id} className={`queue-case${active?.id === c.id ? ' selected' : ''}`} onClick={() => selectCase(c.id)} aria-pressed={active?.id === c.id}><span className="queue-case-top"><span>{c.channel === 'voice' ? '전화' : '웹 접수'} · {c.type === 'missing' ? '미도착' : '오출고'}</span><Status status={c.status}/></span><strong>{c.title}</strong><span className="queue-case-store">{c.store?.name || c.store?.id} · {c.id}</span><span className="queue-case-action">{nextAction(c, role)} <span aria-hidden="true">→</span></span></button>)}</div>
...
function Desk({ caseData: c, mode, fallback, playbackRate, onPlaybackRateChange, onUpdate, onSave, onView, onToast }: { caseData: CaseData; mode: Mode; fallback: boolean; playbackRate: number; onPlaybackRateChange: (rate: number) => void; onUpdate: (c: CaseData) => void; onSave: (id: string, p: Record<string, unknown>) => Promise<CaseData>; onView: (v: View) => void; onToast: (s: string) => void }) {
  ...
  const transferBlockers = [
    ...
    !edited && '접수 정보를 편집·대조해 주세요.',
    !confirmed && '원문 대조·확인 체크를 해 주세요.',
  ].filter((item): item is string => typeof item === 'string');
  const currentStep = deskStep(c, !!analysis, confirmed);
  const stepHint = intakeLocked ? '센터에 전달한 내용과 처리 현황을 확인하세요.' : currentStep === 1 ? (c.channel === 'voice' ? '통화를 끝까지 들은 후 AI 정제를 실행하세요.' : '경영주가 입력한 원문을 읽고 AI 정제를 실행하세요.') : currentStep === 2 ? 'AI 초안을 원문과 대조하고 잘못된 수량·단위와 전달 부서를 바로잡으세요.' : '관련 물류 근거와 미확인 사항을 확인한 후 센터로 전달하세요.';
  return <div className="desk">
    ...
    <nav className="work-navigation" aria-label="상담 처리 흐름">
      <a href="#work-queue" className="queue-return">← 접수 목록으로</a>
      {[{ label: c.channel === 'voice' ? '통화·원문' : '접수 원문', target: 'desk-source' }, { label: '접수 대조·편집', target: 'intake-editor' }, { label: '물류 근거', target: 'desk-evidence' }, { label: '저장·센터 전달', target: 'desk-handoff' }].map((item, index) => <a key={item.target} href={`#${item.target}`} aria-current={(canTransfer ? 4 : currentStep) === index + 1 ? 'step' : undefined}><b>{index + 1}</b>{item.label}<span aria-hidden="true">↓</span></a>)}
    </nav>
    <p className="workflow-hint"><strong>지금 할 일</strong>{stepHint}</p>
    ...
    <div id="desk-source" className="integrated-review task-anchor" tabIndex={-1}>
      <CallReview
        ...
        <button className={!analysis ? "primary" : undefined} disabled={intakeLocked || !!busy || fallback || stale || !!uncertain || !!recovery || (c.channel === 'voice' && !audioEnded)} onClick={() => void analyze()}>{busy === 'analyze' ? 'AI가 전사·정제하고 있습니다…' : mode === 'demo-live' ? 'AI 전사·정제 실행' : '저장된 분석 결과 재생'} <span aria-hidden="true">→</span></button>
