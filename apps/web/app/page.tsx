...
export default function Home() {
  ...
  return <div className="app-shell">
    ...
    <header className="site-header">
      <div className="header-inner"><a className="brand" href="#" onClick={e => { e.preventDefault(); returnToWork(); }} aria-label="HappyCall OneFlow 홈"><span className="brand-mark" aria-hidden="true"><svg viewBox="0 0 24 24" fill="none"><path d="M5 6v12M19 6v12M5 12h14M10 6v12" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"/></svg></span><span>HappyCall <strong>OneFlow</strong></span></a><div className="header-meta"><span className="badge neutral">합성 데이터 시연</span><span className="user-avatar" aria-hidden="true">{role === 'counselor' ? '상' : role === 'center' ? '센' : '점'}</span><span className="user-name">{roleNames[role]} 모드</span></div></div>
      <nav className="nav-inner" aria-label="업무 역할">{tabs.map(tab => <button key={tab.role} onClick={() => changeRole(tab.role)} className={role === tab.role ? 'nav-tab selected' : 'nav-tab'} aria-pressed={role === tab.role}>{tab.label}</button>)}<span className="nav-note">접수 → 확인·이관 → 센터 회신</span></nav>
    </header>
    <main id="main" className="main-content">
      <div className="page-heading"><div><p className="eyebrow">HAPPYCALL WORKSPACE</p><h1>{logistics ? view === 'wms' ? 'WMS 작업 근거' : 'TMS 배송 근거' : tabs.find(t => t.role === role)?.label}</h1><p className="muted">{logistics ? '선택한 접수의 기록을 확인한 뒤 원래 업무로 돌아갑니다.' : role === 'counselor' ? '접수 원문을 확인하고, 정확한 내용과 근거를 센터로 전달하세요.' : role === 'center' ? '이관 내용을 확인하고, 조치 결과를 경영주에게 회신하세요.' : '불편 사항을 접수하고 센터의 회신을 확인하세요.'}</p></div>{role === 'counselor' && !logistics && <div className="mode-control"><label htmlFor="analysis-mode">분석 방식</label><select id="analysis-mode" value={mode} onChange={e => setMode(e.target.value as Mode)}><option value="replay">저장 결과 재생 · API 호출 없음</option><option value="demo-live">실제 AI 분석 · API 사용</option></select></div>}</div>
      ...
      <div className="workspace-sync"><div><strong>{hasDrafts ? '저장하지 않은 초안이 있습니다' : '서버 접수 현황'}</strong><p className="small muted">{hasDrafts ? '업무와 접수를 이동해도 초안을 유지합니다. 페이지를 닫거나 새로고침하면 사라질 수 있습니다.' : '다른 담당자가 등록한 내용은 목록 새로고침으로 확인하세요.'}</p><span className="small muted" role="status">최종 서버 확인 {lastSync || '확인 중'}</span></div><button onClick={() => void reload()} disabled={loading}>{loading ? '최신 내용 확인 중…' : '목록 새로고침'}</button></div>
...
function Desk({ caseData: c, mode, fallback, onUpdate, onSave, onView, onToast }: { caseData: CaseData; mode: Mode; fallback: boolean; onUpdate: (c: CaseData) => void; onSave: (id: string, p: Record<string, unknown>) => Promise<CaseData>; onView: (v: View) => void; onToast: (s: string) => void }) {
  ...
  return <div className="desk">
    ...
    <div className="workflow" aria-label="상담 처리 흐름">{[c.channel === 'voice' ? '통화 확인' : '접수 원문 확인', 'AI 정제·대조', '근거 확인·센터 전달'].map((label, index) => <span key={label} className={currentStep === index + 1 ? 'current' : currentStep > index + 1 ? 'done' : ''} aria-current={currentStep === index + 1 ? 'step' : undefined}><b>{index + 1}</b>{label}</span>)}</div><p className="workflow-hint"><strong>지금 할 일</strong>{stepHint}</p>
    ...
    <section className="panel handoff-panel" aria-label="접수 저장 및 센터 전달"><div><h3>{intakeLocked ? '센터로 전달한 접수입니다' : '확인한 접수를 센터로 전달'}</h3><p className="muted">{intakeLocked ? '이관한 내용은 보존됩니다. 센터의 조치와 회신을 확인하세요.' : '점포·문의 대상·부서와 원문 대조를 확인하세요. 미확인 사항은 남긴 채 전달할 수 있습니다.'}</p></div><div className="panel-actions"><button disabled={intakeLocked || !!busy || fallback || stale || !!uncertain || !!recovery} onClick={() => void persist()}>접수 내용 저장</button><button className="primary" disabled={!canTransfer} onClick={() => void persist(true)}>{busy === 'transfer' ? '전달 중…' : '확인 후 센터 전달'} <span aria-hidden="true">→</span></button></div></section>
  </div>;
}
