...
function Desk({ caseData: c, mode, fallback, playbackRate, onPlaybackRateChange, onUpdate, onSave, onView, onToast }: { caseData: CaseData; mode: Mode; fallback: boolean; playbackRate: number; onPlaybackRateChange: (rate: number) => void; onUpdate: (c: CaseData) => void; onSave: (id: string, p: Record<string, unknown>) => Promise<CaseData>; onView: (v: View) => void; onToast: (s: string) => void }) {
  ...
  const analyzeBlocked = intakeLocked || !!busy || fallback || stale || !!uncertain || !!recovery || (c.channel === 'voice' && !audioEnded);
  ...
  const stepHint = intakeLocked ? '센터에 전달한 접수입니다. 처리 현황을 확인하세요.' : workStage === 'source' ? (c.channel === 'voice' && !audioEnded ? '통화를 끝까지 들으면 AI 정제를 실행할 수 있습니다.' : '원문을 확인한 뒤 AI로 접수 내용을 정리하세요.') : workStage === 'review' ? '필수 정보를 대조하고 원문 확인에 체크해 주세요.' : canTransfer ? '확인한 접수와 근거를 담당 센터에 전달합니다.' : transferBlockers[0];
  return <div className="desk">
    ...
    {workStage !== 'review' && !intakeLocked && !canTransfer && workStage === 'handoff' && <div className="notice warning compact"><span>{transferBlockers.join(' ')}</span><button onClick={() => showStage('review')}>접수서 확인하기</button></div>}
...
function Center({ caseData: c, onSave, onToast, onView, onUpdate, fallback, mode }: { mode: Mode; fallback: boolean; caseData: CaseData; onSave: (id: string, p: Record<string, unknown>) => Promise<CaseData>; onToast: (s: string) => void; onView: (v: View) => void; onUpdate: (c: CaseData) => void }) {
  type CenterDraft = { formRevision?: number; reply: string; pending: string[]; action: string; uncertain?: Record<string, unknown>; operation?: boolean; workStage?: 'evidence' | 'reply' };
  ...
  const generateReply = async () => {
    ...
    try {
      const result = await request<{ replyDraft: string; mode: Mode; revision: number }>(`/api/cases/${c.id}/reply-draft`, 'POST', { mode, expectedRevision: formRevision, centerContext: { pendingActions: pending, reply } }, 'center');
      if (!alive.current) return;
      setGeneratedReply(result.replyDraft); draft.set('workStage', 'reply');
