...
function Scene({ caseData, onBack, onLinkEvidence, backLabel = '상담으로 돌아가기', readOnly: roleReadOnly = false }: Props) {
  ...
  return <section className={styles.scene} aria-label="WMS 공정 확인">
    ...
    <section className={styles.comparison} aria-label="피킹과 출고 기록 비교"><div><p className={styles.eyebrow}>피킹 기록</p><h2>{scan(safePicking)}</h2><p>토트 {label(safePicking.toteId)} · {model.pickingVisible ? date(picking.pickedAt) : '기준시각 내 피킹 시각 미확인'}</p><small>주문 {label(picking.orderId)} · 작업방식 {label(picking.workType)}</small></div><div><p className={styles.eyebrow}>출고 기록</p><h2>{scan(safeShipping)}</h2><p>토트 {label(safeShipping.toteId)} · {model.shippingVisible ? date(shipping.time) : '기준시각 내 출고 시각 미확인'}</p><small>출고 {label(shipping.id)} · 도크 {label(shipping.dock)}</small></div><div className={styles.finding}><strong>{model.difference ? '상품·수량·단위 비교 필요' : caseData.type === 'missing' ? '미수령 진술과 출고 기록을 구분' : '관측값과 미확인을 대조'}</strong><p>{caseData.type === 'missing' ? '경영주는 미수령을 진술했습니다. 센터 출고 기록만으로 점포 도착·인도를 확정할 수 없습니다.' : 'EA와 BOX는 환산하거나 차감하지 않습니다. 피킹·출고 토트의 연결과 차이가 발생한 공정은 추가 확인이 필요합니다.'}</p><b>실제 발생 지점·작업자 귀책 미확인</b></div></section>
    {!!registeredVideos.length && <section className={styles.mediaEntry} aria-label="이 접수에 연결된 영상"><div><strong>기록 차이를 확인한 뒤, 물건의 이동 과정을 살펴보세요</strong><p>피킹·출고 기록과 소터 접근 → 분기 → 슈트 이동을 비교합니다. 발생 위치는 추가 확인이 필요합니다.</p></div>{registeredVideos.map(({ event: sourceEvent, index, result }) => <button key={String(sourceEvent.id)} className={styles.primary} data-testid="case-video-shortcut" onClick={event => { setSelected(index); setPlaying(false); setActive({ clip: result.clip!, event: sourceEvent, opener: event.currentTarget, tracks: result.tracks, processAnchor: result.processAnchor, tracksError: result.tracksError }); }}>▶ 3D 소터 공정 확인</button>)}</section>}
