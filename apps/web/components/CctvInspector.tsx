...
import type { CctvClip, CctvTracks, RegisteredTracks, TrackFrame } from '../lib/cctv-tracks';
...
const phases = [{ key: 'approach', label: '소터 접근' }, { key: 'branch', label: '분기' }, { key: 'chute', label: '슈트 이동' }, { key: 'settle', label: '정지' }] as const;
/** Only already validated, registered frames inside this clip may supply a seek target. */
export function firstPhaseFrame(tracks: CctvTracks | null, clip: Pick<CctvClip, 'startSeconds' | 'endSeconds'>, phase: TrackFrame['phase']) {
  return tracks?.frames.find(frame => frame.phase === phase && frame.elapsedSeconds >= clip.startSeconds && frame.elapsedSeconds <= clip.endSeconds) ?? null;
}
...
function Inspector({ clip, event, picking, shipping, opener, onClose, tracks: descriptor, processAnchor, tracksError: registrationError }: CctvInspectorProps) {
  ...
  function seekPhase(phase: TrackFrame['phase']) {
    const target = firstPhaseFrame(validatedTracks, clip, phase);
    if (!video.current || !target) return;
    video.current.pause(); setPlaying(false); setSelected(true); setOverlay(true); seek(target.elapsedSeconds);
  }
  ...
  return <dialog ref={dialog} className={styles.dialog} aria-labelledby={title} aria-describedby={help} onCancel={e => { e.preventDefault(); close(); }} onClick={e => { if (e.target === e.currentTarget) { const box = e.currentTarget.getBoundingClientRect(); if (e.clientX < box.left || e.clientX > box.right || e.clientY < box.top || e.clientY > box.bottom) close(); } }} onKeyDown={e => {
    ...
    <header className={styles.header}><div><p className={styles.eyebrow}>WMS · Blender 3D 공정 설명</p><h2 id={title}>소터 이동과 원본 기록 비교 <span>· {label(event.label)}</span></h2></div><button type="button" onClick={close} autoFocus aria-label="연결 영상 닫기">닫기 ×</button></header>
    <div className={styles.compactComparison} data-testid="cctv-compact-comparison"><span>대조할 기록</span><strong>피킹 {scan(picking)} → 출고 {scan(shipping)}</strong><small>두 기록의 상품·수량·단위를 확인하세요.</small></div>
    <p id={help} className={styles.notice}>합성 시연 · 실제 CCTV·AI 검출 아님. 이 영상만으로 발생 위치·작업자 귀책을 확정할 수 없습니다.</p>
    <div className={styles.workspace}><section className={styles.main} aria-label="합성 영상과 재생 조작">
      <div className={styles.phaseHeading}><strong>지금 보는 과정 · {phase}</strong><span>단계를 누르면 해당 장면에서 멈춥니다</span></div>
      <div className={styles.phases} role="group" aria-label="3D 공정 구간 탐색">{phases.map((item, index) => <button key={item.key} data-testid={`cctv-phase-${item.key}`} type="button" disabled={!firstPhaseFrame(validatedTracks, clip, item.key)} aria-pressed={current?.phase === item.key} onClick={() => seekPhase(item.key)}><span>{index + 1}</span>{item.label}</button>)}</div>
      ...
      <section className={`${styles.comparison} ${selected ? styles.comparisonSelected : ''}`}><p className={styles.eyebrow}>이 영상에서 확인할 업무</p><h3>상품·수량·단위 비교</h3><div><span>피킹 기록</span><strong>{scan(picking)}</strong><small>토트 {label(picking.toteId)}</small></div><div><span>출고 기록</span><strong>{scan(shipping)}</strong><small>토트 {label(shipping.toteId)}</small></div><p className={styles.warning}>피킹·출고 기록의 차이를 확인하세요. 영상 속 시각 객체와 실제 상품·토트의 연결은 미확인입니다.</p></section>
      ...
      <details className={styles.source}><summary>사건·카메라·기록시각 확인</summary><dl className={styles.context}><div><dt>원본 사건 / 이벤트</dt><dd>{clip.caseId} / {label(event.id)}</dd></div><div><dt>등록 카메라</dt><dd>{clip.cameraId}</dd></div><div><dt>고정 기록시각</dt><dd data-testid="cctv-record-time">{recordTime(clip.occurredAt)}</dd></div></dl></details>
    </aside></div>
  </dialog>;
}
