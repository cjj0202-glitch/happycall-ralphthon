...
PDF는502,292바이트(약0.48MiB)이며 최대5페이지·90MiB 조건 안입니다. 설치된 ReportLab과 맑은 고딕, Poppler로 생성·렌더했습니다. 한글 폰트 내장과 ToUnicode, 58개 텍스트 영역·3개 화면 발췌의 겹침/페이지 경계, 5장 시각 검토를 확인했습니다. 실제 PNG 원본은 그대로 두고 PDF 표시 범위만 pdf-layout.json에 기록했습니다. `rendered/`는 발표자료 검수용이며 새 제품 캡처가 아닙니다.
...
기존 v3에 사용자 부정 청취 피드백 1건이 전달돼 한국어 자연스러움 개선 중임을 PDF 2·5장과 대본·증거표에 명시했습니다. 기술 생성·재생·전사·복구 성공은 자연스러움 통과가 아닙니다. v4는 대본 후보이며 전체 합성·STT·제품 교체는 미완료입니다. 메인 전달 시각을 실제 청취 시각이나 전체 사용자 평가로 확대하지 않습니다. 원래 19개 고정 문서·3개 화면은 유지하고 [후속 입력](feedback-input.json)을 별도로 기록했습니다.
