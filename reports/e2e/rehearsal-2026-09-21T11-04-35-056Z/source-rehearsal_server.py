...
async def main():
    ...
    parser.add_argument("--preserve-state", action="store_true",
                        help="Retain this run's fresh snapshot/state; do not perform recursive cleanup")
    ...
    # Preservation is explicit for reviewed rehearsals; never enumerate/delete old
    # Temp runs. mkdtemp creates a new unique directory and has no cleanup finalizer.
    temporary_context = (nullcontext(tempfile.mkdtemp(prefix="oneflow-rehearsal-"))
                         if args.preserve_state else tempfile.TemporaryDirectory(prefix="oneflow-rehearsal-"))
    with temporary_context as temporary:
        ...
        metadata = {"startedAt": datetime.now(timezone.utc).isoformat(), "pid": __import__("os").getpid(),
                    ...
                    "statePreserved": args.preserve_state,
                    "cleanupPolicy": "preserve-new-run-no-delete" if args.preserve_state else "temporary-directory-default",
