...
const out = path.join(root, 'reports/e2e/prototype-independent-20260921-2102');
...
const server = spawn(path.join(root,'.venv/Scripts/python.exe'), ['tests/helpers/rehearsal_server.py','--port','8911','--metadata',metadataPath,'--preserve-state'], {cwd:root,env:{...env,PYTHONUTF8:'1',PYTHONDONTWRITEBYTECODE:'1'},windowsHide:true,stdio:['ignore','pipe','pipe']});
...
try {
  ...
  browser=await chromium.launch({executablePath:'C:/Users/choi8/AppData/Local/ms-playwright/chromium-1234/chrome-win64/chrome.exe',headless:true});
  const context=await browser.newContext({viewport:{width:1440,height:1000}});
  await context.route('**/*',async route=>{const u=new URL(route.request().url());if(u.hostname!=='127.0.0.1'||u.port!=='8911'){result.external.push(u.origin+u.pathname);return route.abort();}if(u.pathname.endsWith('/analyze'))throw Error('No analysis calls in independent recovery probes');await route.continue();});
