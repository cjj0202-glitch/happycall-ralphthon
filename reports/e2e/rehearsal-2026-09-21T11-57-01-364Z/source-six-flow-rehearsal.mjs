...
const RUNNER_VERSION = 'six-flow-rehearsal-v5-explicit-revision-recovery';
const PORT = Number(process.env.REHEARSAL_PORT || 8821);
assert.ok(Number.isInteger(PORT) && PORT >= 1024 && PORT <= 65532, 'Rehearsal base port must leave four valid ports');
...
const server = spawn(path.join(ROOT, '.venv/Scripts/python.exe'), ['tests/helpers/rehearsal_server.py', '--port', String(PORT), '--metadata', path.join(OUT, 'server.json'), '--preserve-state'],
...
async function waitReady() {
  for (let n = 0; n < 60; n++) {
    ...
    try {
      const r = await fetch(`http://127.0.0.1:${PORT + 3}/api/cases`);
...
async function contextFor(result, base) {
  ...
  await context.route('**/*', async route => {
    ...
    if (u.hostname !== '127.0.0.1' || ![PORT, PORT + 1, PORT + 2, PORT + 3].includes(Number(u.port))) {
...
async function completeBusiness(c, id, result, tools, isVoice) {
  ...
  await stage('review-gates-and-human-handoff', async () => {
    ...
    // WMS/TMS saved newer evidence revisions while the counselor form retained
    // its original revision. A person must compare and choose before editing.
    const savedBeforeChoice = await get(id);
    ...
    await page.getByRole('region', { name: '최신 서버 내용과 초안 대조', exact: true }).waitFor();
    await page.getByRole('button', { name: '서버 내용 사용', exact: true }).click();
    assert.deepEqual(await get(id), savedBeforeChoice, 'Explicit recovery choice must not write to the server');
    assert.equal(await transfer.isDisabled(), true, 'Recovery is not human confirmation');
    result.revisionRecovery = { choice: 'use-server', revision: savedBeforeChoice.revision, serverUnchanged: true };
    const before = await get(id);
    ...
    const comparison = page.locator('details').filter({ has: page.locator('summary', { hasText: 'AI 제안과 현재 입력 상세 대조' }) });
    assert.equal(await comparison.count(), 1, 'One detailed comparison disclosure must remain available');
...
async function flow(round, c, manual = false) {
  ...
  const base = `http://127.0.0.1:${manual ? PORT + 3 : PORT + round - 1}`;
...
try {
  ...
  const t = await contextFor(result, `http://127.0.0.1:${PORT + 3}`); const { page, context, nav } = t;
  try {
