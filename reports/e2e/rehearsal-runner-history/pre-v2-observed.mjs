...
const report = { started: new Date().toISOString(), pc: 'pc1/CJJ', executor: 'AI Playwright, not human observation',
...
try {
  ...
  report.summary = { corePassed: report.core.filter(r => r.status === 'PASS').length, coreExecuted: report.core.length, coreTarget: 6,
    manualPassed: report.textManual.filter(r => r.status === 'PASS').length, manualExecuted: report.textManual.length,
    offlineSafetyPassed: report.offline.filter(r => r.safetyStatus === 'PASS').length, offlineCompletePassed: report.offline.filter(r => r.completeFlowStatus === 'PASS').length,
    externalRequests: report.externalRequests.length, liveRequests: report.liveRequests.length };
  await save(); console.log(JSON.stringify({ out: OUT, ...report.summary, fatal: report.fatal }));
  if (report.fatal || report.summary.corePassed !== 6 || report.summary.manualPassed !== 2 || report.summary.externalRequests || report.summary.liveRequests) process.exitCode = 1;
}
