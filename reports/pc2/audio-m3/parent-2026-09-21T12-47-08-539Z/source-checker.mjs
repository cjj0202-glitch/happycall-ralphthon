...
export async function runChecks({ browser, base, productRoot, out }) {
  ...
  const report = {
    version: 'pc2-m3-parent-replay-v1', started: new Date().toISOString(), productCommit: PRODUCT,
  ...
  try {
    ...
    const formValues = async () => ({
      storeId: await page.getByLabel('점포코드', { exact: false }).first().inputValue(),
      subject: await page.getByLabel('상품·문의 대상', { exact: false }).inputValue(),
      quantity: await page.getByLabel('수령 수량(경영주 진술)', { exact: true }).inputValue(),
      unit: await page.getByLabel('수령 단위', { exact: true }).inputValue(),
      request: await page.getByLabel('요청사항', { exact: true }).inputValue(),
    });
    ...
    await check('analysis-failure-preserves-draft', 'Intentional HTTP503 is visible; actual parent edited form/question and original remain byte-for-byte; retry available', async () => {
      await page.getByLabel('요청사항', { exact: true }).fill('합성 점검 메모: 정정한 1BOX 확인 후 회송 절차 문의');
      await page.getByLabel('추가 질문 메모', { exact: true }).fill('합성 점검 질문: 바깥 박스 라벨을 확인할 수 있나요?');
      const before = await formValues(), question = await page.getByLabel('추가 질문 메모', { exact: true }).inputValue();
      ...
      try {
        ...
        assert.deepEqual(await formValues(), before); assert.equal(await page.getByLabel('추가 질문 메모', { exact: true }).inputValue(), question);
