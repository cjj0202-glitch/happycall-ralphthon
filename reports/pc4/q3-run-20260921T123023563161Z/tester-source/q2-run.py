...
from q2_contract import aggregate_guard, git, inspect_release, inventory, required_inputs, sha
...
def prepare(contract, output):
    result = {'preparedAt': now(), 'task': 'N04-Q2', 'status': 'PREPARED_NOT_RUN',
              'finalInputBlockers': required_inputs(contract), 'productExecutions': 0,
              'finalFlows': {'planned': 6, 'executed': 0, 'passed': 0, 'notRun': 6},
...
def execute(args, contract):
    ...
    out = TEST_ROOT / 'reports/pc4' / ('q2-final-' + stamp)
    logs = TEST_ROOT / '.local' / ('q2-final-' + stamp)
    out.mkdir(parents=True, exist_ok=False)
    logs.mkdir(parents=True, exist_ok=False)
    report = {'startedAt': now(), 'task': 'N04-Q2', 'status': 'NOT_RUN',
              'productRoot': str(product), 'testerHead': git(TEST_ROOT, 'rev-parse', 'HEAD'),
              'runnerHashes': {p.name: digest(p) for p in Path(__file__).parent.glob('*.py')},
              'checkerHash': digest(Path(__file__).with_name('flow-ui-check.mjs')),
    ...
    try:
        ...
        import uvicorn
        holder = {}
        owned = Path(tempfile.mkdtemp(prefix='pc4-q2-final-'))
        ...
        with contextlib.nullcontext(stack):
            ...
            command = [str(node), str(Path(__file__).with_name('flow-ui-check.mjs'))]
        ...
        report['runnerHashesEnd'] = {p.name: digest(p) for p in Path(__file__).parent.glob('*.py')}
        report['checkerHashEnd'] = digest(Path(__file__).with_name('flow-ui-check.mjs'))
...
def main():
    ...
    parser.add_argument('--output', type=Path, default=TEST_ROOT / 'reports/pc4/q2-preparation.json')
