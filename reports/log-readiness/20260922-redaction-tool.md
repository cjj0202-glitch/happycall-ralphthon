...
- 일반 문자열에서 OpenAI/GitHub/AWS 형식, URL 자격증명·Bearer·유효한 Base64 Basic 인증·JWT, 이메일·국내 휴대폰 후보, inline data URL을 종류별 표식으로 바꿉니다. 순수 Base64는 media 유형 문맥의 `data/base64/b64_json/payload/image/audio/video/url` 필드에서만 정제하며 일반 alt/caption은 이 규칙에서 제외합니다. 자격증명·이메일 경계는 ASCII 문자집합을 사용해 한글에 인접한 값을 놓치지 않습니다. 형태 일치는 실제 계정 소유 여부를 검증한 뜻이 아닙니다.
...
timestamp/time/date/datetime, id 및 `_id`/`_ids`/`_timestamp`, type/role/name/model/effort/reasoning_effort, created_at/updated_at/started_at/completed_at/finished_at, duration_ms/status/namespace/comp_hash/sender/recipient 계열은 보호합니다. 보호 문자열에 알려진 패턴/정책 해시가 있으면 원문을 유지하고 `UNRESOLVED_PROTECTED`로 집계합니다. 없는 ID/필드는 생성하지 않습니다. `encrypted_content`는 검사·치환하지 않고 `OPAQUE_UNREVIEWED` 수로 남깁니다. 오류의 구조·유형·상태는 보호하며 자유문자열 오류 message는 일반 정제 대상입니다.

한 사건씩 streaming 처리하며 메모리 상한은 가장 큰 사건 크기에 좌우됩니다. 큰 data URL을 먼저 분리해 다른 자격증명 정규식이 payload를 재검사하지 않습니다. 전체 파일을 메모리에 읽지 않습니다.

## 합성 검증 결과

명령은 위 `unittest discover`이며 최종 **20 tests: 19 통과, 1 skipped**, 0.390초였습니다. 원본 로그·실제 키·기존 비공개 후보·원장은 열지 않았고 합성 fixture만 사용했습니다.
...
**변이 대조 7/7 killed**: 마스킹 제거, 보호 필드 가드 제거, 기록 구조 가드 제거, 중복 key 거부 제거, media 필드 제한 제거, 의심 key 미해결 가드 제거, OpenAI 앞 경계를 Unicode `\w`로 회귀. 변이마다 같은 로딩 경로의 비변이 CONTROL을 먼저 통과시켜 위치/의존성 실패와 구분했습니다.
...
실제 입력 실행 전 독립 검토자는 최종 script SHA `e89b3c2bea79efca5b94def111dc044d6f6d4c36607e087ed51bd8e3603064d5`에서 자기 합성 입력으로 11형식×단독/한글왼쪽/한글오른쪽 **33/33** 마스킹·출력/감사 비노출을 확인했습니다. 설명 보존·payload 제거, 민감 key 원문 보존+미해결1, 상태/ID/Goal/boolean/null/암호문 보존도 확인했습니다. 이번 보완3개를 메모리에서 되돌린 변이는 원본PASS/변이FAIL **3/3**이었습니다. 정책/비일치/보호/감사8조건 **8/8**, 실패경계7개 **7/7**에서 exit2·완료manifest0을 관측했습니다. 소스 전후 해시는 같았고 실제 로그·키·네트워크는 읽지 않았습니다. 지정한 NOT-APPROVED 로컬 후보 생성 범위로 인수합니다.

메인이 실제 고정 snapshot에 도구를 실행하고 독립 비교기로 보존/정제를 대조해야 합니다. 알려진 패턴과 등록 해시의 일치는 자유서술 전체의 공개 허가를 뜻하지 않습니다. 정책 누락은 `NOT_PROVIDED`, 정책 제공은 `HASH_MATCHES_ONLY`, 의미 검토는 `NOT_PERFORMED`로 남깁니다. 새 회사정보·이름/주소·모든 연락처 형식·암호문 의미·다른 PC 중복·HowLong 수용·최종 제출은 확인하지 않았습니다.
